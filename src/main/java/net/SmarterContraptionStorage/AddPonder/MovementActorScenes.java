...
public class MovementActorScenes {
    public static void controlStorageBlock(SceneBuilder scene, SceneBuildingUtil util){
        ...
        scene.overlay.showText(70).pointAt(util.vector.topOf(chest.west(2)).add(0,-0.15,0)).placeNearTarget().text("Items will be stored in contraptionLink's container automatically");
        scene.idle(5);
        ...
        scene.idle(50);
        scene.overlay.showText(70).pointAt(util.vector.topOf(control).add(0,-0.25,0)).placeNearTarget().text("Using this mod, you could control every container behaviour on contraptionLink. Here let's use drawer right click controls block to set its value");
        scene.addKeyframe();
        scene.idle(70);
        ...
        scene.idle(35);
        scene.overlay.showText(70).placeNearTarget().pointAt(util.vector.topOf(control).add(0,-0.25,0)).independent(60).text("If we release piston now,the chest won't be added to contraption. So these logs will drop on the ground");
        scene.idle(80);
        scene.effects.indicateRedstone(lever);
        scene.world.toggleRedstonePower(util.select.fromTo(lever,gearshift));
        scene.world.modifyKineticSpeed(kinetics,(speed) -> -speed);
