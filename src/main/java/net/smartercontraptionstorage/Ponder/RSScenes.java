...
public class RSScenes {
    public static void useRS(SceneBuilder builder, SceneBuildingUtil util) {
        ...
        scene.idle(110);
        scene.overlay().showControls(util.vector().topOf(importBus), Pointing.DOWN,50).withItem(Items.INSTANCE.getWirelessGrid().getDefaultInstance());
        scene.overlay().showControls(util.vector().topOf(exportBus), Pointing.DOWN,50).withItem(Items.INSTANCE.getWirelessGrid().getDefaultInstance());
